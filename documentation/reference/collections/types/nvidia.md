...
| `cvelist` | `list[str]` | Related CVE identifiers referenced by this document. | `["CVE-2026-47476", "CVE-2026-47477", "CVE-202…` |
...
| `epss` | `list[object{cve,date,epss,percentile}]` | EPSS exploitation-probability forecast datapoints (score + percentile). | `[{"cve": "CVE-2026-47476", "date": "2026-07-1…` |
| `href` | `str` | Canonical URL of the document at its original source. | `"https://nvidia.custhelp.com/app/answers/deta…` |
| `id` | `str` | Unique document identifier (e.g. a CVE id, exploit id or advisory id). | `"NVIDIA:5853"` |
| `lastseen` | `str` | Last time Vulners observed/refreshed the document (ISO-8601). | `"2026-07-15T15:37:34"` |
...
| `timestamps` | `object{contentUpdated,created,enriched,metricsUpdated,reviewed,updated}` | Vulners lifecycle timestamps (created/updated/enriched/reviewed/…). | `{"created": "2026-07-14T11:43:19.132000Z", "u…` |
| `title` | `str` | Human-readable title of the document. | `"Security Bulletin: NVIDIA Triton Inference S…` |
| `type` | `str` | Source collection the document comes from (cve, exploitdb, ubuntu, …). | `"nvidia"` |
| `vhref` | `str` | URL of the document on vulners.com. | `"https://vulners.com/nvidia/NVIDIA:5853"` |
...
Specific to the `nvidia` collection.
