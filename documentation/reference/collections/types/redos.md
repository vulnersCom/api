...
| `id` | `str` | Unique document identifier (e.g. a CVE id, exploit id or advisory id). | `"ROS-20260720-73-0067"` |
| `lastseen` | `str` | Last time Vulners observed/refreshed the document (ISO-8601). | `"2026-07-20T13:14:11"` |
...
| `timestamps` | `object{contentUpdated,created,enriched,metricsUpdated,reviewed,updated}` | Vulners lifecycle timestamps (created/updated/enriched/reviewed/…). | `{"created": "2026-07-20T13:14:11.411000Z", "u…` |
| `title` | `str` | Human-readable title of the document. | `"ROS-20260720-73-0067"` |
| `type` | `str` | Source collection the document comes from (cve, exploitdb, ubuntu, …). | `"redos"` |
| `vhref` | `str` | URL of the document on vulners.com. | `"https://vulners.com/redos/ROS-20260720-73-0067"` |
...
| `description` | `str` | Full text or summary of the vulnerability/advisory. | `"The vulnerability of the WebCodecs component…` |
| `enchantments` | `object{dependencies,score,short_description,tags}` | Vulners-computed enrichment layer (AI score, tags, related docs). | `{"score": {"value": 4.8, "uncertanity": 1.2, …` |
| `reporter` | `str` | Person or organization credited with reporting/authoring it. | `"Redos"` |

### Collection fields

Specific to the `redos` collection, beyond the common and family sets.
...
| `cvelist` | `list[str]` | Related CVE identifiers referenced by this document. | `["CVE-2026-7933"]` |
