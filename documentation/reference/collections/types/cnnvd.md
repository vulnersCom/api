...
**Family model:** [`AdvisoryBulletin`](../../data-models.md) — `bulletinFamily: cnnvd`. Fields are grouped by where they appear across the samples; anything Vulners adds beyond the models stays accessible via `extra="allow"`.
...
| `lastseen` | `str` | Last time Vulners observed/refreshed the document (ISO-8601). | `"2026-07-21T04:43:20"` |
...
Present in every sampled `cnnvd`-family document (typed by [`AdvisoryBulletin`](../../data-models.md)).
...
Specific to the `cnnvd` collection, beyond the common and family sets.
