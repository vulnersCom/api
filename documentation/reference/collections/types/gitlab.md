...
| `cvelist` | `list[str]` | 85% | Related CVE identifiers referenced by this document. | `["CVE-2026-50274"]` |
...
| `cvss3` | `object{cvssV31}` | 50% | CVSS v3.x score block. | `{"cvssV31": {"source": "security-advisories@g…` |
...
| `enchantments` | `object{dependencies,score,short_description,tags}, object{score,short_description,tags}` | 100% | Vulners-computed enrichment layer (AI score, tags, related docs). | `{"score": {"value": 6.1, "uncertanity": 2.5, …` |
| `epss` | `list[object{cve,date,epss,percentile}]` | 50% | EPSS exploitation-probability forecast datapoints (score + percentile). | `[{"cve": "CVE-2026-50274", "date": "2026-07-1…` |
...
| `metrics` | `object{cna}` | 50% | Raw scoring metrics blob (CNA/ADP/NVD/vendor sub-objects). | `{"cna": {"cvss31": {"source": "security-advis…` |
...
| `timestamps` | `object{contentUpdated,created,enriched,metricsUpdated,reviewed,updated}` | 100% | Vulners lifecycle timestamps (created/updated/enriched/reviewed/…). | `{"created": "2026-07-16T06:18:13.758000Z", "u…` |
