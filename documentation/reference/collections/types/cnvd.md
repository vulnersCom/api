...
**Family model:** [`AdvisoryBulletin`](../../data-models.md) — `bulletinFamily: cnvd`. Fields are grouped by where they appear across the samples; anything Vulners adds beyond the models stays accessible via `extra="allow"`.
...
Present in every sampled `cnvd`-family document (typed by [`AdvisoryBulletin`](../../data-models.md)).
...
Specific to the `cnvd` collection, beyond the common and family sets.
