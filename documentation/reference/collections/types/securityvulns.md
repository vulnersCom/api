...
|---|---|---|---|---|
| `affectedSoftware` | `list[object{name,operator,version}]` | 35% | Affected software products (name/version/operator). | `[{"operator": "eq", "version": "0.3", "name":…` |
...
| `enchantments` | `object{aggregatedScoring,backreferences,dependencies,score,short_description,tags}, object{backreferences,dependencies,score,short_description,tags}` | 100% | Vulners-computed enrichment layer (AI score, tags, related docs). | `{"score": {"value": 3.1, "vector": "NONE"}, "…` |
...
| `references` | `list[str]` | 35% | External reference URLs. | `["https://vulners.com/securityvulns/securityv…` |
...
| `timestamps` | `object{contentUpdated,created,enriched,metricsUpdated,reviewed,updated}` | 100% | Vulners lifecycle timestamps (created/updated/enriched/reviewed/…). | `{"created": "2015-11-01T21:00:00Z", "updated"…` |
