...
|---|---|---|---|---|
| `affectedSoftware` | `list[object{name,operator,platform,type,vendor,version,versionRaw}]` | 85% | Affected software products (name/version/operator). | `[{"vendor": "\u041e\u041e\u041e \u00ab\u0420\…` |
| `bulletinFamily` | `str` | 100% | Broad family the document belongs to (cve, exploit, software, …). | `"info"` |
| `cpeConfigurations` | `object{vulnersCpeConfiguration}` | 65% | CPE applicability configurations (NVD-style match tree). | `{"vulnersCpeConfiguration": [{"operator": "OR…` |
| `cvelist` | `list[str]` | 80% | Related CVE identifiers referenced by this document. | `["CVE-2025-8715"]` |
...
| `cvss2` | `object{cvssV2}` | 100% | CVSS v2 score block. | `{"cvssV2": {"source": "vendor", "version": "2…` |
| `cvss3` | `object{cvssV3}` | 100% | CVSS v3.x score block. | `{"cvssV3": {"source": "vendor", "version": "3…` |
| `description` | `str` | 95% | Full text or summary of the vulnerability/advisory. | `"The vulnerability of the pg_dump utility in …` |
| `enchantments` | `object{dependencies,score,short_description,tags}, object{score,short_description,tags}` | 100% | Vulners-computed enrichment layer (AI score, tags, related docs). | `{"score": {"value": 7.0, "uncertanity": 1.7, …` |
| `epss` | `list[object{cve,date,epss,percentile}]` | 45% | EPSS exploitation-probability forecast datapoints (score + percentile). | `[{"cve": "CVE-2025-8715", "date": "2026-07-16…` |
...
| `references` | `list[str]` | 80% | External reference URLs. | `["https://www.postgresql.org/support/security…` |
...
| `timestamps` | `object{contentUpdated,created,enriched,metricsUpdated,reviewed,updated}` | 100% | Vulners lifecycle timestamps (created/updated/enriched/reviewed/…). | `{"created": "2026-06-30T12:57:05.185000Z", "u…` |
