...
| `metrics` | `object{adp,cna}, object{adp,nvd}, object{cna,nvd}, object{nvd}` | Raw scoring metrics blob (CNA/ADP/NVD/vendor sub-objects). | `{"cna": {"cvss4": {"version": "4.0", "vectorS…` |
...
| `references` | `list[str]` | External reference URLs. | `["https://lxr.openwrt.org/ident?i=blobmsg_for…` |
...
Specific to the `openwrt` collection.
